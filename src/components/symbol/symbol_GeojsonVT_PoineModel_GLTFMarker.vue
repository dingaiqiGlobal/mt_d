...
export default {
  ...
  mounted() {
    ...
    this.gltfLayer = new GLTFLayer("gltfLayer");
    this.groupLayer.addLayer(this.gltfLayer, "gltfLayer");
  ...
  methods: {
    addGltfMarker(data) {
      ...
      const gltfMarker = new GLTFMarker(data, {
        ...
        .then((json) => {
          GeoJSON.toGeometryAsync(json).then((geos) => {
            geos.map((p) => {
              ...
              let data = [coordinate.x, coordinate.y];
              this.addGltfMarker(data)
            });
          });
        });
    },
